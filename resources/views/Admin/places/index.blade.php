...
@section('title') {{$setting->title}} | Places @endsection
@section('page_name') Places @endsection
@section('css')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{$setting->title}} Places</h3>
                    <div class="">
...
@section('ajaxCalls')
    <script>
        ...
        showData('{{route('places.index')}}', columns);
        deleteScript('{{route('places.delete')}}');

        // Get Edit View
        $(document).on('click', '.editBtn', function () {
            var id = $(this).data('id')
            var url = "{{route('places.edit',':id')}}";
        ...
        $(document).on('click', '.addBtn', function () {
            ...
            $('#editOrCreate').modal('show')
            setTimeout(function () {
                $('#modalContent').load('{{route('places.create')}}')
            }, 250)
        });
        ...
        $(document).on('submit','Form#updateForm',function(e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status == 200){
                        $('#dataTable').DataTable().ajax.reload();
                        toastr.success('Place updated successfully');
                    }
                    else
                        toastr.error('There is an error');
