...
@section('title') {{$setting->title}} | OBSTACLE COURSES @endsection
@section('page_name') OBSTACLE COURSES @endsection
@section('css')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{$setting->title}} OBSTACLE COURSES</h3>
                    <div class="">
                        <button class="btn btn-secondary btn-icon text-white addBtn">
									<span>
										<i class="fe fe-plus"></i>
									</span> Add OBSTACLE COURSES
                        </button>
                    </div>
                </div>
                <div class="card-body">
...
@section('ajaxCalls')
    <script>
        ...
        showData('{{route('obstacleCourses.index')}}', columns);
        deleteScript('{{route('obstacleCourses.delete')}}');

        // Get Edit View
        $(document).on('click', '.editBtn', function () {
            var id = $(this).data('id')
            var url = "{{route('obstacleCourses.edit',':id')}}";
        ...
        $(document).on('click', '.addBtn', function () {
            ...
            $('#editOrCreate').modal('show')
            setTimeout(function () {
                $('#modalContent').load('{{route('obstacleCourses.create')}}')
            }, 250)
        });
