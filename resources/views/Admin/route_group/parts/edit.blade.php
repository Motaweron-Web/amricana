...
<div class="modal-header">
    <h5 class="modal-title" id="example-Modal3">Edit activity for group</h5>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
    </button>
</div>
<div class="modal-body">
    <form id="updateForm" method="POST"  action="{{route('routes.update',$route->id)}}" >
    @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <select name="group_id" class="form-control select2" data-placeholder="Choose Group">
                @foreach($groups as $group)
                    <option value="{{$group->id}}" {{$route->activity_id == $group->id ? 'selected' : ''}}>{{$group->title}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="activity_id" class="form-control select2" data-placeholder="Choose Activity">
                @foreach($activities as $activity)
                    <option value="{{$activity->id}}" {{$route->activity_id == $activity->id ? 'selected' : ''}}>{{$activity->title}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="time" class="form-control" name="time_group" id="time_group" value="{{$route->time_group}}">
        </div>
