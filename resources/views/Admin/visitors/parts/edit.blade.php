...
<div class="modal-body">
    ...
    @csrf
        ...
        <div class="row">
            ...
            {{--start visitor places--}}
            <div class="col-6">
                <div  class="form-group">
                    <label class="form-label">Assign Places</label>
                    <select name="visitor_place_id[]" class="form-control select2" data-placeholder="Choose Places" multiple>
                        @foreach($places as $place)
                            <option value="{{$place->id}}" {{ in_array($place->id,$visitor_place)? 'selected' : ''}}>{{$place->title}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
