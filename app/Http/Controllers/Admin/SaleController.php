        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($tickets)
                ...
                ->addColumn('cancel', function ($tickets) {
                    ...
                    return '<span style="cursor:pointer" class="icon btn btn-danger cancel" data-bs-toggle="tooltip">Canceled</span>';
                    else
...
//                        return $ticketSales->ticket_price + $ticketSales->vat + $ticketSales->discount_value;
                ...
                ->addColumn('discount_amount', function ($ticketSales){
                    ...
                    if ($ticketSales->discount_type == 'per') {
                        return (round($ticketSales->total_price - $ticketSales->grand_total , 2));
                    } elseif ($ticketSales->discount_type == 'val')
    ...
    public function attendanceReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($ticketModels)
                ...
                ->addColumn('Theater', function ($ticketModels) {

                    $place = VisitorPlace::where('title','=','Theater')->first();
                    return DB::table('visitor_places_models')
                        ->where('visitor_type_id','=',$ticketModels->visitor_type_id)
                        ->where('visitor_place_id','=',$place->id)->count() * $ticketModels->total;

                })
                ->addColumn('Musium', function ($ticketModels) {

                    $place = VisitorPlace::where('title','=','Musium')->first();
                    return DB::table('visitor_places_models')
                            ->where('visitor_type_id','=',$ticketModels->visitor_type_id)
                            ->where('visitor_place_id','=',$place->id)->count() * $ticketModels->total;
                })
                ->addColumn('Kimit', function ($ticketModels) {

                    $place = VisitorPlace::where('title','=','Kimit')->first();
                    return DB::table('visitor_places_models')
                            ->where('visitor_type_id','=',$ticketModels->visitor_type_id)
                            ->where('visitor_place_id','=',$place->id)->count() * $ticketModels->total;
                })

                ->addColumn('Cinma', function ($ticketModels) {

                    $place = VisitorPlace::where('title','=','Cinma')->first();
                    return DB::table('visitor_places_models')
                            ->where('visitor_type_id','=',$ticketModels->visitor_type_id)
                            ->where('visitor_place_id','=',$place->id)->count() * $ticketModels->total;
                })
